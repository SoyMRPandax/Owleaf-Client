...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Conectando...", "author": "beyong" },
            { "message": "Conectando...", "author": "beyong" },
            { "message": "Conectando...", "author": "beyong" }
        ];
    ...
    async checkUpdate() {
        this.setStatus(`error al buscar actualización...`);
        ...
        ipcRenderer.on('update-not-available', () => {
            console.error("Actualización disponible");
    ...
    shutdown(text) {
        this.setStatus(`${text}<br>Cerrar Launcher en 5 segundos`);
        let i = 4;
        setInterval(() => {
            this.setStatus(`${text}<br>Quedarse ${i--}s`);
